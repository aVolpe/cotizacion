SELECT  qrd.id,  p,  br, qr.id, qr.date, qrd.id, qrd.sale_price, qrd.purchase_price 
...
AND qr.id IN (SELECT MAX(qr2.id) FROM query_response qr2 WHERE qr2.branch_id = br.id)  
